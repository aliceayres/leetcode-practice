...
join Employee more on emp.departmentId = more.departmentId
where more.salary >= emp.salary
group by dpt.Name,emp.Name having count(distinct more.salary)<=3
order by dpt.id asc,emp.salary desc

# ① count(distinct more.salary)<=3
# ② join Employee more on emp.departmentId = more.departmentId
"""