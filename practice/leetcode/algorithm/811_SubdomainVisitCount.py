...
class Solution:
    def subdomainVisits(self, cpdomains):
        ...
        result = []
        for cp in cpdomains:
            splits = cp.split(' ') # split
            current = splits[1]
            count = int(splits[0])
            cache[current] = cache.get(current,0) + count
            while True:
                list = current.split('.',1)
                if len(list)==1:
                    break;
                cache[list[1]] = cache.get(list[1],0)+count
                current = list[1]
        for item in cache.items():
            dm = str(item[1]) + " " +item[0]
            result.append(dm)
        return result

    def transform(self,cpdomains):
        rs = []
        for cp in cpdomains:
            splits = cp.split(' ')
            current = splits[1]
            count = int(splits[0])
            rs.append((count,current))
        rs.sort(key=lambda item : item[1]) # sort
        return rs
